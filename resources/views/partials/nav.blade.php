<nav class="navbar navbar-expand-lg navbar-light ">
    ...
    <section class="d-flex gap-1">
       ...
       <a class="nav-link border bg-seconary text-dark  p-1" href="#"><strong>{{ auth()->user()->login }}</strong></a>
       <a class="nav-link border bg-seconary p-1 " href="{{ route('logout') }}">Logout</a>
      
      @endif
      @if (auth()->guest())
        <div class="{{ setActivo('login') }} nav-item">
          <a class="nav-link border bg-seconary text-dark  p-1" href="{{ route('login') }}">Login</a>
        </div>
      @endif
    </section>
